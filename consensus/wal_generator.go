...
func WALGenerateNBlocks(t *testing.T, wr io.Writer, numBlocks int, chainID string) (err error) {
	...
	blockExec := sm.NewBlockExecutorRaw(chainID, stateStore, log.TestingLogger(), proxyApp.Consensus(), mempool, evpool)
	consensusState := NewState(config.Consensus, state.Copy(), blockExec, blockStore, mempool, evpool, chainID)
...
func WALWithNBlocks(t *testing.T, numBlocks int, chainID string) (data []byte, err error) {
	var b bytes.Buffer
	wr := bufio.NewWriter(&b)

	if err := WALGenerateNBlocks(t, wr, numBlocks, chainID); err != nil {
		return []byte{}, err
	}
